...
- `js/data/metadata.js` — заметки и исторические декларации дизайна (`D.designHistory`);
...
состояния. [Контракт материалов](material-contract.md) проверяет используемые блоки
и стены перед построением кешей и в CLI. Инспектор использует те же правила и
показывает диагностику повреждённой записи. Полный предметный контракт остаётся
в #27; реестр отрисовки описан ниже. Геометрия и игровые имена пока не подтверждаются.

`scene-audit.js` объединяет чистые проверки и независимые требования в один отчёт.
`populateSceneAudit()` вычисляет его один раз; `renderSceneAudit()` выводит ошибки,
предупреждения и цели раздельно. [Модель и CLI](computed-audit.md).
...
сохраняются при рефакторинге. `*-extensions.js` регистрируют собственные пары kind/style в `OBJECT_RENDERERS`.
`rendererForObject()` используется и проверкой, и отрисовкой; запасного display
для неизвестных типов нет. [Подробности и границы](object-renderers.md).
Объектные диспетчеры больше не подменяют друг друга; подмены функций текстур
блоков/стен пока сохранены до #33.
