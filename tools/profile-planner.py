...
def one_profile(browser, browser_session, origin, entry, device, repeat, output):
    ...
    try:
        ...
        page.evaluate('__profile.api.fit(); __profile.api.state.cam.scale=4')
        # fit schedules a frame; consume it before measuring input work.
        settle(page)
        phases = {}
        if not mobile:
            box = page.locator('#viewport').bounding_box()
            def hover():
                for i in range(80):
                    page.mouse.move(box['x']+15+(i*47)%(box['width']-30),
...
def main():
    ...
    report = {'schemaVersion':1,'status':'FAIL','commit':commit,'applicationSha256':application_digest(),
